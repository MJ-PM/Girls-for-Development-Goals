...
export function Projects() {
  ...
  return (
    <div className="w-full grid grid-cols-1 md:grid-cols-12 auto-rows-auto gap-4">
      {/* Header Bento Box */}
      <section className="col-span-1 md:col-span-12 bg-white rounded-3xl p-8 md:p-12 border border-slate-200 text-center">
        <h1 className="font-serif text-4xl md:text-5xl lg:text-6xl font-bold text-indigo-950 mb-6">Field Initiatives</h1>
      ...
      <section className="col-span-1 md:col-span-12 bg-white rounded-3xl p-6 md:p-10 border border-slate-200 flex flex-col min-h-[500px]">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-8 gap-6 border-b border-slate-200 pb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 grow">
          <AnimatePresence mode="popLayout">
            {filteredProjects.map((project) => (
              <motion.div layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.3 }} key={project.title} className="p-6 border border-slate-200 bg-slate-50 rounded-2xl flex flex-col hover:border-indigo-300 hover:shadow-lg transition-all group">
                <div className="flex justify-between items-start mb-4">
                  <span className={cn("text-[10px] font-bold px-2 py-1 rounded-md uppercase", project.status === "Completed" ? "bg-indigo-100 text-indigo-700" : "bg-orange-100 text-orange-700")}>{project.status}</span>
                  <span className="text-slate-400 text-[10px] uppercase font-bold tracking-widest">{project.category}</span>
                </div>
                <h4 className="font-bold text-xl text-indigo-950 mb-3 font-serif leading-tight group-hover:text-indigo-600 transition-colors">{project.title}</h4>
                <p className="text-slate-600 text-sm mb-6 flex-grow leading-relaxed">{project.summary}</p>
                <button className="text-xs font-bold uppercase tracking-wider text-indigo-600 hover:text-orange-600 transition-colors flex items-center gap-1.5 w-fit">
                  Case Study <ArrowRight className="w-3.5 h-3.5 transition-transform group-hover:translate-x-1" />
                </button>
              </motion.div>
            ))}
          </AnimatePresence>
